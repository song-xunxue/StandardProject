...
# Dependencies: flask, flask-cors, cozepy, langchain, langchain-community, langchain-openai, python-dotenv, bcrypt, PyJWT
python coze_app.py
```
...
- `app/services/auth_service.py` — JWT + bcrypt authentication (in-memory user store)
...
- `app/services/memory_store.py` — In-memory data store (knowledge bases, conversations, API keys, presets)
