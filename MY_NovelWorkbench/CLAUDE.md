...
**当前状态：v2 三批三项全部完成（2026-09-24 收官，329 用例全绿，三视角审查 16 项发现 6 确认全修复）——遗留修复（快照恢复统计对账 recomputeChapterChars / F16 单路浮层→角落状态条 BeatStatusBar）/ F15 拆书冷启动（LLM 逐章抽取实体→确认→addNodesBatch 入库）/ F18 导出管线（TXT 粘贴档+MD 回流档零依赖 / EPUB/DOCX 可选 Pandoc）；拍板与验收标准见 `docs/03-下阶段优化方案.md` 第 11 节；F15 需真实 API Key 联调、F18 EPUB/DOCX 需本机 Pandoc 实测（留档未做项见 11.4）；四批候选（F10/F14/F9 LLM 摘要档/F11/F12/性能项）待用户挑选。**
...
npm run test       # vitest 单测（纯逻辑 + jsdom 编辑器桥接，当前 329 用例）
...
- `services/fileService.ts` — 文件树/蓝图/章节 CRUD（`resolveInNovel` 路径穿越防护）+ 卷支持（chapters/ 一层卷目录，章节按「第N章」数字序）+ exchangeFiles（文件名互换=章节排序）；rename 同步内部 title（蓝图 JSON / 章节 frontmatter）；readChapter 读取边界归一换行（外部导入 CRLF 无 frontmatter 章节的下游段落切分依赖）
...
- `services/statsService.ts` — 码字统计（v2-F7）：writing-stats.json（chapterChars 按路径记账 + days 每日总量）；openNovel 全量对账/saveChapter 入账/deleteFile 清账/renameFile 键迁移/exchangeFiles 账值对调；recomputeChapterChars 全量重算（v2 三批：全本快照恢复后对账，ipc snapshotRestore 编排，days 历史保留）；tmp+rename 原子写
- `services/wordbankService.ts` — 敏感词词库（v2-F6）：userData/sensitive-words/<名>.json，列表/保存（去空去重原子写）/删除/导入 txt（每行一词，覆盖或并入）；不内置词条
- `services/exportService.ts` — 导出服务（v2-F18，纯 node 可单测）：scope 三态（全本/按卷/选章）经 readTree 单一真相排序展开→逐章 readChapter→shared/exportFormat 纯函数拼装→tmp+rename 原子落盘；EPUB/DOCX 走 md 中间件+Pandoc（发布态覆写：剥双链+标题必开+title/author/lang 元数据；spawn 参数数组中文路径安全；60s 超时；fake runner 注入单测；检测会话级缓存）；保存对话框在 ipc 层（exportNovel 一站式，取消返回 null）
...
- `layout/StatsPanel.tsx` — 码字统计浮层（今日/总量/连续天数三卡片+近14天柱图；负增益 sign-aware）；`layout/ExportPanel.tsx` — 导出浮层（v2-F18：范围三态×格式四选×选项+Pandoc 置灰降级+作者输入写回 novel.json；Esc 过滤输入控件与 IME 组合态）；`layout/SensitivePanel.tsx` — 敏感词浮层（站点库管理/当前章与全本检测——激活章用编辑草稿/按词聚合命中+跳章）；浮层族统一遮罩点关+Esc
...
- `canvas/BeatLauncher.tsx` — 节拍整章发起浮层（F16：节拍勾选/字数档位/目标章当前或新章/三路候选；单路发起成功即关浮层——写入器生命周期在 beatSingleSession，进度转 BeatStatusBar）；`canvas/BeatStatusBar.tsx` — 单路生成角落状态条（v2 三批：实时字数/停止/完成/中断/失败提示；done 态 5s 自动淡出；App 根级常驻挂载会话态驱动显隐）；`canvas/ExtractPanel.tsx` — 拆书冷启动浮层（v2-F15：左章节进度/右候选勾选与行内编辑（稳定 id 作 key）/目标图选择（既有或新建拆书蓝图 defaultTitle 去重）/类别标签幂等创建/addNodesBatch 单笔入库；Esc 走 handleClose 候选丢失确认；别名草稿失焦解析）
- `services/contextAssembly.ts` + `graphTraversal.ts` — 上下文组装与图遍历纯函数（分支覆盖 97.3%；**关键词兜底只扫草稿尾部 KEYWORD_SCAN_TAIL_CHARS=20000 字**）；`naming.ts` — 默认标题去重；`recapAssembly.ts` — 前情双档组装（F9：总量上限均摊）；`generateMessages.ts` — 续写/改写消息组装（F13 预设替换式注入）；`beatAssembly.ts` — 节拍排序（arrow 拓扑+并列 y/x 稳定+容环回退）与整章消息组装（F16，不走三层预算）；`extractAssembly.ts` — 拆书逐章抽取 prompt（严格 JSON 指令+已知名单回灌对齐称谓，EXTRACT_MAX_TOKENS=4096，v2-F15）；`beatSingleSession.ts` — 单路节拍会话控制器（v2 三批遗留修复）：F16 单路写入器生命周期宿主（模块级单例——「关闭浮层≠中断生成」由架构保证），自建 chunk 订阅按 requestId 捕获 done 包（sawDone 终态判定：外部中断统一标 stopped 不误报完成），BeatStatusBar 角落状态条的数据源
