...
| 2026-09-01 | v2.4 | v2 首批阶段三四 + 晨间全面测试优化批次收官：**阶段三（696507a）**：F5 情节结构模板（资源库第三类 structure：节点序列+索引连线插入骨架；内置三幕/英雄之旅/救猫咪；「存当前图为结构模板」整图压骨架；晨间修复：种子改 .structures-seeded 标记驱动——原按目录存在性判定 v2 升级用户拿不到内置骨架）+ F3 三路候选续写（delta 路由 Map 化按 requestId 分路；multiGen 三路并发 80ms 节流累积；A/B/C 分栏流式、采纳一路 markdown 插入、全停/放弃；与单会话互斥+双卸载守卫）。**阶段四（d73788d）**：F4 时间线矩阵（行=引用节点标签/列=章节数字序/格=交叉点亮，伏笔行红色；点击格子跳章即关覆盖层——晨间修复与全局图谱行为对齐）。**晨间批次（四维 44 代理审查：19 项确认全修复）**：①数据正确性——统计键随重命名迁移/随交换对调（防双计与重启负增量）、近14天 carry 初值（新用户前13天不再显示今日总量）、全停丢节流窗尾部（flush 后重取 state）、采纳走 markdown 重排+发起章节校验（防跨章误插）、统计文件原子写；②交互——AI 面板卸载即收尾（防段落压扁）、角标移出省略号容器（长标题可见）、敏感词全本检测覆盖编辑中草稿、快照恢复不再拷回统计历史；③一致性——结构模板种子标记、浮层族统一 Esc+onMouseDown 关闭、左栏 footer 两行防溢出、面板宽度特异性修复、11 文件头部日志补记；④打磨——插入骨架后 fitView+选中+关浮层+包围盒落点（防重叠/视口外不可见）、startGeneration 互斥守卫、done 包带 delta 先转发、类型单一来源。213 用例全绿（+5），CDP 全功能冒烟 11/11（F4 端到端造引用节点→指向→贴伏笔→矩阵点亮→跳章），零 console 错误 |
| 2026-09-18 | v2.5 | v2 二批收官（四阶段 + 五维审查 + CDP 冒烟 10/10）：范围拍板 F8+F9+F13+F16（docs/03 第 10 节验收标准落档）。**①F13 去 AI 味改写预设**——资源库第四类 rewritePreset（内置「去AI味」独立标记 .rewrite-presets-seeded 种子，删光不复活）；预设指令替换式注入（非叠加，避免与默认指令矛盾）、scope 限定改写操作；资源库 PresetForm 新建/编辑 + AI 面板选择器。**②F9 前情提要双档**——NovelMeta.recap 可选字段（尾部全文/开头摘要 × N 章 × 每章字数，渲染层兜底默认=旧行为）；组装抽纯函数 recapAssembly（RECAP_MAX_TOTAL_CHARS 总量均摊防超支）；Context Viewer 显示档位与前情 token（治 system 尾部隐形消耗）。**③F8 章节级快照 + diff**——.snapshots/chapters/<章路径 base64url>/ 分组隔离（全本列表/prune 天然不混入、每章独立 20 份、上限常量上移 shared）；快照体=整文件原文、恢复整文件直写（零 frontmatter 伪变更）+ recordChapterSave 入账；行级 LCS diff 纯函数（shared/chapterDiff：前后缀快进/超规模降级/ctx 折叠）+ diff 浮层（红删绿增/折叠跳行/一键恢复）；轻量恢复编排（chapterFlush→IPC→chapterReloadSeq）；TabBar 章节 Tab 右键入口；顺带修复全本快照创建缺前置冲刷的存量缺口；countChars 上移 shared/textMetrics（快照服务保持零 electron 依赖）。**④F16 场景节拍→整章草稿**——planBeats 节拍排序纯函数（子图 arrow 边拓扑序 ADR-15 + 并列拍画布 y/x 稳定排序 + 无边/成环回退坐标序 + F1 never 过滤）；beatChapterMessages 组装（上层设定链+逐拍设定+前情+字数指令；不走三层预算）；maxTokens 按字数档位 1.2 倍放开透传；multiGen 扩展 kind/originPath/targetTitle（chapterDraft 会话不依赖编辑器——ChapterEditor/AiPanel 卸载停生成豁免）；BeatLauncher 发起浮层（勾选节拍/字数档位/目标章当前或新章/单路流式或三路）；新章目标采纳时自动建章（createFile 返回实际路径，不产废弃空章）；候选区抽出 MultiCandidates 独立组件（AI 面板内 + 根级全局浮层双挂载）。**⑤遗留修复**——addNodesBatch 批量建点+索引连线（单次 set 单笔落盘：消除逐个 addNode 的首笔部分写崩溃窗口与 N 次全图拷贝 churn；结构模板插入迁移）；GlobalGraphView 渲染代际守卫（genRef 代际号 + chainRef 串行队列：G6 异步操作严格串行、旧代链空跑、全链 try/catch；setData 首帧烤入过滤/高亮治一帧闪烁）。五维审查（17 代理 + 反驳式复核：1 high + 4 medium + 12 low/info 全修复，1 项误报被复核剔除——BeatLauncher 遮罩挡图标条触发链不可达）：high=F16 上层设定链补 F1 never 过滤（planAncestors 纯函数+UI 提示+回归用例）；medium=BeatLauncher Esc 守卫 busy 化、waitForEditor 删 null 豁免（防采纳文本被编辑器加载覆盖静默丢失）、浮层 Esc 层级管理（diff/Dialog 互斥）、maxTokens 系数 1.5+llmService clamp；low=摘要层真首段/快照恢复 tmp+rename/入账容错/读侧校验/预设改名删旧档+resourceVersion 同步/指令 8000 上限/对比前冲刷等。268 用例全绿（+55），typecheck/build 全绿，CDP 冒烟 10/10 零 console 错误（F16 节拍拓扑序/F8 快照-diff-恢复全链路/F9 双档注入/F13 种子/addNodesBatch 实弹）。**排障注记**：本机 Node 24.11.1 的 rmSync 对非 ASCII 目录 recursive 删除存在静默漏删变体（删部分文件无异常不崩溃）——测试环境清理改逐文件 unlinkSync + rmdir |
| 2026-09-24 | v2.6 | v2 三批收官（三阶段 + 三视角审查 16 项发现 6 确认全修复 + 分域三次审查修复提交；范围拍板=遗留修复+F15+F18，docs/03 第 11 节）：**①遗留修复（a481334）**——快照恢复统计对账：实证修正五维审查留档认知（死键实际会被 initStats 清理，真缺陷是 live 键值不校正永不自愈），statsService.recomputeChapterChars 按磁盘内容全量重算 chapterChars（days 历史保留，ipc snapshotRestore 恢复成功后/openNovel 前调用+失败兜底分支同调）；F16 单路浮层角落状态条：写入器生命周期迁入 beatSingleSession 模块级单例，发起成功即关浮层，进度转 App 根级 BeatStatusBar（补齐中断入口/失败提示两个缺口）；StatsPanel 负增益 sign-aware。**②F15 拆书冷启动（2ec47d9）**——shared/extract 纯函数（LLM 输出容错解析：围栏/噪声/类型漂移/坏条目跳过/截断括号栈抢救任意层自闭合对象；跨章去重合并不做单向包含防误并；既有节点匹配；超长章 2 万字段落边界切窗）；extractAssembly 逐章 prompt（严格 JSON 指令+已知名单回灌）；extractStore 自有 extract- 前缀+自建 chunk 订阅绕开 aiStore 单会话互斥（长批次不锁死续写，双向过滤成立）；ExtractPanel 浮层（章节进度/候选勾选行内编辑/目标图选择）；入库=类别幂等建标签+addNodesBatch 单笔落盘。**③F18 导出管线（82e2942）**——shared/exportFormat 纯函数（TXT 粘贴档：段间单空行/块间双空行/CRLF/可选 BOM 与章标题行/wikilink 与 Markdown 记号纯文文化——晋江实证「缩进平台自动调」；MD 回流档：卷#/章##/双链保留）；exportService（readTree 单一真相排序展开/逐章读取/tmp+rename 原子落盘/EPUB-DOCX 走 md 中间件+Pandoc 发布态覆写（剥双链+标题必开+title/author/lang 元数据）+fake runner 可注入单测）；IPC export:checkPandoc+exportNovel（一站式 showSaveDialog+落盘）；novelStore.exportNovel 前置 chapterFlush（防抖窗口正文必进导出）；ExportPanel 浮层（范围×格式×选项+Pandoc 置灰降级+作者 NovelMeta 新增可选字段写回）。**④审查修复（5342ee2/302f76a/fad432e）**——high=CRLF 无 frontmatter 导入章节读取边界归一（fileService.readChapter，治 TXT 段落坍缩+孤立 \r，一处修复惠及导出/拆书/前情）；medium×5=ExtractPanel 稳定 id（治名称输入每键失焦+勾选集三连失真）/Esc 改走 handleClose（候选丢失确认）/别名草稿失焦解析（分隔符不再被吞）、beatSingleSession sawDone 自然完成判定（外部中断不再误报已完成）、未分卷空卷块间距；low 顺手修=会话代际号防 reset 重开竞态/重试错误提示/新图 defaultTitle 去重/探测期禁用 EPUB/ExportPanel Esc 输入守卫。329 用例全绿（+61），typecheck/build 全绿，双平台已推。**验收提示**：F15 需真实 API Key 联调（抽取-确认-入库闭环）；F18 EPUB/DOCX 需本机装 Pandoc 实测（TXT/MD 零依赖已单测覆盖） |
